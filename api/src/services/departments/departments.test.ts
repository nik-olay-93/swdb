  ...
  scenario('creates a department', async (scenario: StandardScenario) => {
    const result = await createDepartment({
      input: {
        name: 'String',
        description: 'String',
        facultyId: scenario.department.two.facultyId,
        updatedAt: '2022-11-24T21:16:47.080Z',
      },
    })

    expect(result.name).toEqual('String')
    expect(result.description).toEqual('String')
    expect(result.facultyId).toEqual(scenario.department.two.facultyId)
    expect(result.updatedAt).toEqual(new Date('2022-11-24T21:16:47.080Z'))
  })
